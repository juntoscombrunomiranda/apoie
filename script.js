...
let frameAzulImg, frameVerdeImg;
...
const initCanvas = () => {
  ...
  stage.on('wheel', (e) => {
    ...
    photo.x(
      pointer.x -
      mousePointTo.x * photo.scaleX()
    );

    photo.y(
      pointer.y -
      mousePointTo.y * photo.scaleY()
    );
...
frameSwitch.forEach(input => {
  ...
  input.addEventListener('change', () => {
    ...
    if (input.value === 'azul') {
      ...
      if (frameAzulImg) {
        frameAzulImg.visible(true);
      }

      if (frameVerdeImg) {
        frameVerdeImg.visible(false);
      }

    } else {

      if (frameAzulImg) {
        frameAzulImg.visible(false);
      }

      if (frameVerdeImg) {
        frameVerdeImg.visible(true);
      }
...
canvasContainer.addEventListener('touchmove', (e) => {
  ...
  if (lastDistance) {
    ...
    photo.x(
      centerX -
      (centerX - photo.x()) *
      (photo.scaleX() / oldScale)
    );

    photo.y(
      centerY -
      (centerY - photo.y()) *
      (photo.scaleY() / oldScale)
    );
...
window.addEventListener('resize', () => {
  ...
  if (frameAzulImg) {
    ...
    frameAzulImg.width(newSize);
    frameAzulImg.height(newSize);

  }

  if (frameVerdeImg) {

    frameVerdeImg.width(newSize);
    frameVerdeImg.height(newSize);
